...
class User extends Authenticatable
{
    ...
    {
        // Use Wallet as source of truth (negative balance = debt)
        $wallet = \DB::table('wallets')->where('student_id', $this->id)->first();

        if (!$wallet) {
            // No wallet = no debt (backward compatible with legacy behavior)
            return 0;
        }

        $balance = (float) $wallet->balance;

        // Negative balance means student owes money (outstanding debt)
        // Positive or zero balance means no outstanding debt
