...
class BillingService
{
    ...
    public function billTerm($sessionId, $semesterId, $processedByUserId)
    {
        ...
        // Fixed: Role check should match "Student" (RoleSeeder) or "student" (legacy)
        // Fixed: Check both Spatie roles AND legacy role column to ensure all students are caught
        $students = User::where(function ($q) {
            $q->role(['Student', 'student']) // Spatie Scope
                ->orWhereIn('role', ['Student', 'student']); // Legacy Column
        })
            ->where('status', 'active')
                ...
                DB::transaction(function () use ($student, $studentFeesToApply, $batch, $sessionId, $semesterId, &$processedCount, &$totalAmount) {
                    foreach ($studentFeesToApply as $feeTemplate) {
                        ...
                        $studentFee = StudentFee::create([
                            ...
                            // 'class_id' => $feeTemplate->class_id, // Removed: Column does not exist in DB
                            'session_id' => $sessionId,
                            'semester_id' => $semesterId,
                            'amount' => $feeTemplate->amount,
                            'description' => $feeTemplate->description ?? "Termly school fee bulk billing",
                            'status' => 'unpaid', // Fixed: Fee is an invoice (unpaid), Wallet Charge is the debt
                            'amount_paid' => 0,
                            'balance' => $feeTemplate->amount,
