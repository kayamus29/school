...
class AcademicSettingController extends Controller
{
    ...
    public function showTotalSchoolDaysForm()
    {
        $current_school_session_id = $this->getSchoolCurrentSession();
        $semesters = $this->semesterRepository->getAll($current_school_session_id);
        ...
        return view('academics.total-school-days', $data);
    }

    public function updateTotalSchoolDays(Request $request)
    {
        $request->validate([
            'semester_id' => 'required|integer|gt:0',
            'total_school_days' => 'required|integer|min:0',
        ]);

        try {
            $this->semesterRepository->update($request->semester_id, ['total_school_days' => $request->total_school_days]);

            return back()->with('status', 'Total school days updated successfully!');
        } catch (\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
