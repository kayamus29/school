...
class PaymentController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            'student_id' => [
                ...
                function ($attribute, $value, $fail) {
                    ...
                    if (!$user || !in_array($user->role, ['student', 'Student'])) {
        ...
        try {
            ...
            $this->walletService->deposit(
                $request->student_id,
                $request->amount_paid,
                'student_payment', // Fixed: Use short name to match system convention
                $payment->id,
                $description
            );
    ...
    public function getStudentDetails($id)
    {
        ...
        if (!in_array($student->role, ['student', 'Student'])) {
            return response()->json(['error' => 'Selected user is not a student.'], 403);
        }
