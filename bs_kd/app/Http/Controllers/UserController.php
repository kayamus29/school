...
class UserController extends Controller
{
    ...
    public function showExportForm()
    {
        ...
        return view('students.export', $data);
    }
    ...
    public function exportStudents(Request $request)
    {
        ...
        if ($class_id == 0) {
            $students = $this->userRepository->getAllStudentsBySession($this->getSchoolCurrentSession());
        } else {
            $students = $this->userRepository->getAllStudents($this->getSchoolCurrentSession(), $class_id, $section_id);
        }

        $students->load('schoolClass', 'section', 'student');
        ...
        $callback = function () use ($students, $columns) {
            ...
            foreach ($students as $student) {
                ...
                foreach ($columns as $column) {
                    switch ($column) {
                        case 'id_card_number':
                            $row[] = $student->id_card_number ?? '';
                            break;
                        case 'first_name':
                            $row[] = $student->student->first_name ?? '';
                            break;
                        case 'last_name':
                            $row[] = $student->student->last_name ?? '';
                            break;
                        case 'email':
                            $row[] = $student->student->email ?? '';
                            break;
                        case 'birthday':
                            $row[] = $student->student->birthday ?? '';
                            break;
                        case 'address':
                            $row[] = $student->student->address ?? '';
                            break;
                        case 'phone':
                            $row[] = $student->student->phone ?? '';
                            break;
                        case 'gender':
                            $row[] = $student->student->gender ?? '';
                            break;
                        case 'class':
                            $row[] = $student->schoolClass->class_name ?? '';
                            break;
                        case 'section':
                            $row[] = $student->section->section_name ?? '';
                            break;
                        default:
                            $row[] = $student->{$column} ?? '';
                            break;
