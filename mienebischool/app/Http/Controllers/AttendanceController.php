...
class AttendanceController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            $attendance_type = optional($academic_setting)->attendance_type == 'section' ? 'section' : 'course';
            $existing_attendances = $attendanceRepository->getTodayAttendance(
                $class_id, 
                $section_id, 
                $course_id, 
            ...
            $data = [
                ...
                'course_id' => $course_id,
                'class_name' => optional($school_class)->class_name,
                'section_name' => optional($school_section)->section_name,
                'existing_attendances' => $existing_attendances, // Pass existing attendances
            ];
    ...
    public function store(AttendanceStoreRequest $request)
    {
        ...
        try {
            $academic_setting = $this->academicSettingRepository->getAcademicSetting(); // Need academic_setting to determine attendance_type
            $attendanceRepository = new AttendanceRepository();

            // Determine attendance type for deletion
            $attendance_type = optional($academic_setting)->attendance_type == 'section' ? 'section' : 'course';

            // Delete existing attendance for today before saving new ones
            $attendanceRepository->deleteTodayAttendance(
                $request->class_id, 
                $request->section_id, 
                $request->course_id, 
                $request->session_id, 
    ...
    public function show(Request $request)
    {
        ...
        try {
            ...
            if ($course_id) {
                ...
                // If section attendance, try to find semester by date range
                $semesters = $this->semesterRepository->getAll($current_school_session_id);
                foreach ($semesters as $sem) {
                    // Check if current date is within semester start and end dates
                    if (now()->between($sem->start_date, $sem->end_date)) {
                        $semester = $sem;
                        break;
                    }
                }
