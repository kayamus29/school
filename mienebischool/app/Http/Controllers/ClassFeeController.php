...
class ClassFeeController extends Controller
{
    ...
    public function generateBills(Request $request)
    {
        ...
        try {
            // Removed strict check for existing batch to allow re-runs for new students.
            // Service layer handles idempotency (skips students already billed).
            // $existing = BillingBatch::where(...) -> exists();
    ...
    public function getBulkPreviewAjax(Request $request)
    {
        ...
        // Fetch all classes. We no longer eager load students as it was incorrect.
        ...
        foreach ($classes as $class) {
            ...
            // BUG-008 Fix: Count students from promotions for the selected session, not historical class members.
            $studentCount = \App\Models\Promotion::where('session_id', $request->session_id)
        ...
        $request->validate([
            'source_semester_id' => [
                ...
                function ($attribute, $value, $fail) use ($request) {
                    // FIX-014: Ensure source semester belongs to the current session
                    $semester = \App\Models\Semester::find($value);
                    if ($semester && $semester->session_id != $request->session_id) {
                        $fail('The source semester must belong to the selected session.');
                    }
                }
            ],
            'target_semester_id' => [
                ...
                'exists:semesters,id',
                function ($attribute, $value, $fail) use ($request) {
                    // FIX-014: Ensure target semester belongs to the current session
                    $semester = \App\Models\Semester::find($value);
                    if ($semester && $semester->session_id != $request->session_id) {
                        $fail('The target semester must belong to the selected session.');
                    }
                }
            ],
            'session_id' => 'required|exists:school_sessions,id',
