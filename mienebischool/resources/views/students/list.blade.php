                    ...
                    <div class="col ps-4">
                        <h1 class="display-6 mb-3">
                            ...
                            @if(Auth::user()->hasRole('Teacher'))
                                @if(isset($context) && $context === 'marks')
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    @if(Auth::user()->hasRole('Teacher'))
                                        {{ ($context === 'marks') ? 'My Course Students' : 'My Class Students' }}
                                    @else
                                        Student List
                                    @endif
                                </li>
                        ...
                        <div class="mb-4 mt-4">
                            <form class="row" action="{{route('student.list.show')}}" method="GET">
                                ...
                                <div class="col">
                                    ...
                                    @role('Admin')
                                    <a href="{{route('student.export.show')}}" role="button" class="btn btn-outline-secondary ms-2"><i class="bi bi-download"></i> Export Students</a>
                                    @endrole
                                    @if(Auth::user()->hasRole('Teacher') && !empty($class_id) && $class_id > 0)
                                        <a href="{{route('attendance.create.show', [
                                            'class_id' => $class_id,
                                            'section_id' => $section_id ?? 0
                                        ])}}" role="button" class="btn btn-outline-primary ms-2"><i class="bi bi-calendar2-check"></i> Take Attendance</a>
                                    @endif
                                </div>
                            </form>
                                    ...
                                    <tbody>
                                        @foreach ($studentList as $student)
                                            <tr>
                                                ...
                                                <td>{{$student->student->parent_info->guardian_email ?? 'N/A'}}</td>
                                                <td>{{$student->student->parent_info->guardian_phone ?? 'N/A'}}</td>
                                                <td>
                                                    <div class="btn-group" role="group">
                                                        @if(Auth::user()->hasRole('Teacher'))
                                                            @if(isset($context) && $context === 'marks')
                                                                {{-- Marks Context Actions --}}
                                                                <a href="{{route('course.mark.create', [
                                                                    'class_id' => $student->class_id,
                                                                    'section_id' => $student->section_id,
                                                                    'course_id' => $course_id
                                                                ])}}" role="button" class="btn btn-sm btn-outline-success"><i class="bi bi-input-cursor"></i> Give Marks</a>
                                                                ...
                                                                ])}}" role="button" class="btn btn-sm btn-outline-info"><i class="bi bi-cloud-sun"></i> View Results</a>
                                                            @else
                                                                {{-- Attendance Context Actions --}}
                                                                <a href="{{route('student.attendance.show', ['id' => $student->student->id])}}" role="button" class="btn btn-sm btn-outline-secondary"><i class="bi bi-clock-history"></i> History</a>
                                                            @endif
                                                        @else
                                                            {{-- Default/Admin Actions --}}
                                                            <a href="{{route('student.attendance.show', ['id' => $student->student->id])}}" role="button" class="btn btn-sm btn-outline-primary"><i class="bi bi-clock-history"></i> Attendance</a>
                                                        @endif
