...
const ClassFeeManager = {
    ...
    async loadBulkPreview() {
        ...
        // Fix: Use specific selector to avoid grabbing empty input from manageFeeForm
        const sessionId = document.querySelector('#wizard-step-1 input[name="session_id"]').value;

        if (!semesterId) {
            this.showToast('Please select a term first', 'warning');
            this.wizardStep = 1;
            this.updateWizardUI(); // Ensure UI syncs
            return;
        }
        ...
        try {
            ...
            if (response.data.success) {
                // Fix: Populate the hidden form field with the selected semester ID
                document.getElementById('final_semester_id').value = semesterId;

                this.renderBulkPreview(response.data);
            }
        } catch (error) {
            console.error('Preview Error:', error);
            const msg = error.response?.data?.message || error.message || 'Error generating preview. Check console.';
            this.showToast(msg, 'danger');

            // Go back to Step 1, but safely
            this.wizardStep = 1;
            this.updateWizardUI();
        }
    },
